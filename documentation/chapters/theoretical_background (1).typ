...
$ r(t) = s(t) + n(t) $ #cite(<proakis2008digital>)
...
$ h(t) = s(T_b - t) $ #cite(<proakis2008digital>)
...
$ P_b = Q!(sqrt(2 E_b slash N_0)) $ #cite(<proakis2008digital>)

where $Q(x) = frac(1, sqrt(2 pi)) integral_x^infinity e^{-u^2 slash 2} d u$ is the Gaussian tail function #cite(<proakis2008digital>). This closed-form expression is the benchmark against which the project's simulated BER-vs-SNR curves (produced by the Python modeling suite) are compared, and against which the measured coding/processing gain of the spreading system is quantified.
...
$ s[n] = b dot c[n], quad n = 0, dots, N_c - 1 $ #cite(<pickholtz1982theory>)

where $N_c$ chips are transmitted per data bit. In hardware this multiplication is implemented as an XNOR between the data bit and the LFSR-generated chip, which is logically equivalent to $plus.minus 1$ multiplication on bipolar values — the basis of the Transmitter module in this project. In this project, $N_c = 1023$, matching the GPS C/A code length discussed below.

=== Processing gain

The ratio of chip rate to bit rate defines the *processing gain*:

$ G_p = frac(R_c, R_b) = frac(T_b, T_c) = N_c $ #cite(<pickholtz1982theory>)
...
$ M_j ["dB"] = G_p ["dB"] - (E_b slash N_0)_"req" ["dB"] - L_"impl" ["dB"] $ #cite(<kaplan2006understanding>, supplement: "Ch. 4")
...
$ hat(b) = "sign"( sum_{n=0}^{N_c-1} r[n] med c[n] ) $ #cite(<pickholtz1982theory>)
...
$ D = frac(|R_E| - |R_L|, |R_E| + |R_L|) $ #cite(<kaplan2006understanding>, supplement: "Ch. 5") #cite(<teunissen2017springer>, supplement: "Ch. 24")
...
$ N = 2^n - 1 $ #cite(<golomb1967shift>)

M-sequences have three properties (the "balance", "run", and "correlation" properties) that make them suitable spreading codes #cite(<golomb1967shift>):
...
This project uses a 10-bit LFSR, yielding $N = 1023$ chips per period, the basis of the PRNS generator used both in the FPGA transmitter/receiver and in the Python reference model for offline BER and tracking analysis. This is deliberately the same code length used by GPS for its civilian C/A code: each satellite is assigned a distinct 1023-chip Gold code, formed by modulo-2 adding two 10-bit m-sequences (registers G1 and G2, with feedback polynomials $1+x^3+x^10$ and $1+x^2+x^3+x^6+x^8+x^9+x^10$ respectively) at a chip rate of 1.023 Mchip/s #cite(<kaplan2006understanding>, supplement: "Ch. 4"), chosen precisely for their controlled auto- and cross-correlation properties (the "Gold bound" of $-23.9$ dB) #cite(<kaplan2006understanding>, supplement: "Ch. 4").

=== Multiple access and the near–far problem

Because distinct users' codes have low mutual cross-correlation, several DSSS transmissions can share the same frequency band simultaneously (Code-Division Multiple Access) with each receiver's correlator suppressing the others as pseudo-noise — in GPS, the 32 satellite Gold codes are chosen so that their pairwise cross-correlation never exceeds the Gold bound of $-23.9$ dB relative to the autocorrelation peak #cite(<kaplan2006understanding>, supplement: "Ch. 4"). This robustness is not unconditional, however: if an interfering transmitter's received power is much larger than the desired signal's, residual cross-correlation sidelobes can still swamp the wanted correlation peak — the classical *near–far problem* — which is why power control or attenuation matching matters in the channel emulation stage of this project's three-board test setup.
